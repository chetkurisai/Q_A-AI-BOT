...
if not api_key:
    st.error("⚠️ GROQ_API_KEY not found. Please set it in your environment or .env file.")
    st.stop()
...
st.sidebar.image(
    "https://upload.wikimedia.org/wikipedia/commons/thumb/a/af/PDF_file_icon.svg/1024px-PDF_file_icon.svg.png",
...
if uploaded_file:
    ...
    # Input box for a new query (unique key each time)
    query = st.text_input(
        "🔍 Enter your query", 
        key=f"query_{len(st.session_state.conversation)}"
    )

    if st.button("💬 Get Answer", key=f"button_{len(st.session_state.conversation)}"):
        if query:
    ...
    if st.session_state.conversation:
        ...
        for i, qa in enumerate(st.session_state.conversation, 1):
            st.write(f"**Q{i}:** {qa['query']}")
